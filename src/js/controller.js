...
class Controller {
    ...
    this.player.template.playedBarWrap.addEventListener(utils.nameMap.dragMove, (e) => {
      if (this.player.video.duration) {
        ...
        const time =
          ...
          (tx / this.player.template.playedBarWrap.offsetWidth);
        if (utils.isMobile) {
          this.thumbnails && this.thumbnails.show();
        }
        this.thumbnails && this.thumbnails.move(tx);
        this.player.template.playedBarTime.style.left = `${
          tx - (time >= 3600 ? 25 : 20)
        }px`;
  ...
  initPlayedBar1() {
    const thumbMove = (e) => {
      let percentage =
        ((e.clientX || e.changedTouches[0].clientX) -
          utils.getBoundingClientRectViewLeft(this.player.template.controller.progress)) /
        this.player.template.controller.progress.clientWidth;
      percentage = Math.max(percentage, 0);
      percentage = Math.min(percentage, 1);
      this.player.bar.set('barPlayed', percentage, 'width');
      this.player.template.controller.time.innerHTML = utils.secondToTime(
        percentage * this.player.video.duration
      );
    };
    ...
    const thumbUp = (e) => {
      ...
      this.player.template.controller.barPlayed.setAttribute(
        'width',
        `${percentage.toFixed(0)}px`
      );
      this.player.bar.set('barPlayed', percentage, 'width');
    ...
    this.player.template.controller.progress.addEventListener(
    ...
    this.player.template.controller.progress.addEventListener(
      utils.nameMap.dragMove,
      (e) => {
        if (this.player.video.duration) {
          const px =
            this.player.template.controller.progress.getBoundingClientRect().left;
          const tx = (e.clientX || e.changedTouches[0].clientX) - px;
          ...
          const time =
            this.player.video.duration *
            (tx / this.player.template.controller.progress.offsetWidth);

          this.player.template.controller.time.style.left = `${
            tx - (time >= 3600 ? 25 : 20)
          }px`;
  ...
  initDanmakuButton() {
    ...
    this.player.template.controller.danmakuBtn.addEventListener('click', () => {
      console.log(111);
      a = !a;
      this.player.template.controller.danmakuIcon.innerHTML = a
        ? Icons.danmuOn
        : Icons.danmuOff;
    });
  }
  ...
  initVolumeButton1() {
    ...
    this.player.template.controller.volumeProgress.addEventListener('click', (event) => {
      ...
      let percentage =
        ((e.clientX || e.changedTouches[0].clientX) -
          utils.getBoundingClientRectViewLeft(
            this.player.template.controller.volumeProgress
          ) -
          0) /
        vWidth;
      percentage = Math.max(percentage, 0);
      ...
      this.player.template.controller.volumeProgress.setAttribute(
        'aria-label',
        `音量 ${pw}`
      );
    });
    ...
    this.player.template.controller.volumeProgress.addEventListener(
      utils.nameMap.dragStart,
      () => {
  ...
  setAutoHide() {
    ...
    this.autoHideTimer = setTimeout(() => {
      if (
        this.player.video.played.length &&
        !this.player.paused &&
